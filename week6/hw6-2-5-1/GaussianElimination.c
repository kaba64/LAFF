...
int GaussianElimination_unb( FLA_Obj A );
int ForwardSubstitution_unb( FLA_Obj A, FLA_Obj b );
...
int main( int argc, char *argv[] ) {
  ...
  //FLA_Obj_show( " A = [ ", C, "%le", " ];" );
#endif
  printf("The spent time for computing  Gaussian elimination of A in second : %lf\n",t2-t1);
  FLA_Obj_free(&A);
  FLA_Obj_free(&b);
  //FLA_Obj_free(&x);
  FLA_Finalize();
...
int ForwardSubstitution_unb( FLA_Obj A, FLA_Obj b )
{
  ...
  while ( FLA_Obj_length( ATL ) < FLA_Obj_length( A ) ){
    ...
    /*------------------------------------------------------------*/
    AXPY_Gaussian_Rank1_unb(beta1,a21,b2);
    /*------------------------------------------------------------*/
